package com.example.fibath.ui.Printer.sunmi.v2.utils;

import android.graphics.Bitmap;

import java.io.ByteArrayOutputStream;

// Common instruction package
public class ESCUtil {
	...
	/**
	 * Print a single QR code sunmi custom instruction
	...
	/**
	 * Two horizontal QR code sunmi custom instructions
	...
	/*
	*	Select bitmap instruction set mode
	...
	* Single-byte mode is off
	...
	private static byte[] setQRCodeSize(int modulesize){
		// QR code block size setting instruction
	...
	private static byte[] setQRCodeErrorLevel(int errorlevel){
		// Two-dimensional code error correction level setting instruction
		byte[] dtmp = new byte[8];
		dtmp[0] = GS;
		dtmp[1] = 0x28;
		dtmp[2] = 0x6B;
	...
	private static byte[] getBytesForPrintQRCode(boolean single){
		...
		byte[] dtmp;
		if(single){		// Print only one QRCode on the same line, followed by a newline
			dtmp = new byte[9];
			dtmp[8] = 0x0A;
		}else{
			dtmp = new byte[8];
